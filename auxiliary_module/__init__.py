...
class NonogramFrame2(pd.DataFrame):
    ...
    def __init__(self, shape: tuple = None):
        ...
        super().__init__(columns = ())
        for k in range(1,shape[0]+1):
            for m in range(1,int(np.ceil(shape[1]/2))+1):
                self['row_' + str(k) + '_' + str(m)] = np.nan
        for k in range(1,shape[1]+1):
            for m in range(1,int(np.ceil(shape[0]/2))+1):
                self['col_' + str(k) + '_' + str(m)] = np.nan
...
def set_shape(shape_: tuple[int,int]) -> None:
    """
    Set the shape that will be used across the entire module auxiliary_module.
