...
class TuningHyperModelRandom(kt.HyperModel):
    ...
    def fit(self,hp: kt.HyperParameters,model,val_data,val_target,**kwargs):
        ...
        min_delta = hp.Float("min_delta",min_value=1e-6,max_value=1e-2,sampling="log")
        ...
        return model.fit(data,target,epochs=1_000,
...
for i in range(5,50):
    ...
    search_tuner = kt.RandomSearch(TuningHyperModelRandom(),objective="val_loss",max_trials=2,
    ...
    val_target = val_target.to_numpy().reshape((val_target.shape[0],1,val_target.shape[1]))
